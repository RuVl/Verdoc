...
- **The stock forecast is about now, not the period.** "Will it last" must not depend on which range somebody is browsing, so it always uses the last 30 days of sales against `Product.objects.with_available()`.
...
- **Chart.js 4.5.1 is vendored** in `sales/static/sales/` (MIT, one UMD file) - the admin must not call a CDN. Its palette is read from the admin's CSS variables, so the theme toggle carries the chart with it; update it by replacing the file. Data rides in via `json_script`, there is no API endpoint. The CSV export starts with a BOM or Excel mangles non-ASCII product names.
