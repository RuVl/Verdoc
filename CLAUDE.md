...
make sync-transactions c="--dry-run"  # diff transactions against the Plisio API; c="--discover --days 7 --order N"
...
6. **Reconciliation** - `sync_transactions` (`make sync-transactions`) asks Plisio what it thinks of our invoices, because a callback that was lost or arrived while we were down leaves a paid order in PENDING with nothing handed over. `sales/plisio.py` is the read side of the API (`GET /operations/<txn_id>`, tried against both shops' keys since a row does not record which one minted it) plus `apply_order_status()`, the one place an invoice status turns into stock movement - the callback view calls the same function, so the two cannot drift. Rules: only what Plisio actually said is written, so a field the operations endpoint omits keeps the value the richer callback payload carried; a **paid order is never un-sold** by a status that maps to PENDING (a currency switch leaves a "cancelled duplicate" invoice behind, and the callback can follow it only because it sees invoices as they happen); **one dead unpaid state does not become another** (Plisio stores an unpaid invoice as `cancelled` long after telling us `expired` - the units went back when the order died and EXPIRED says why, so the row takes Plisio's word and the order keeps ours); an invoice naming another order is reported and never repaired. An invoice minted before a shop's API key was replaced answers 404 on every key we still hold - the `txn_id` is a Mongo ObjectId, so `int(txn_id[:8], 16)` dates it and shows the cut-off. `--dry-run` reports only, `--raw` prints Plisio's answer verbatim (an odd status usually explains itself in a field we do not read), `--discover` also walks Plisio's invoice list for invoices we have no row for at all, and a repaired sale is delivered and mailed like a real callback (`--no-email`, `--skip-orders` to hold back).
