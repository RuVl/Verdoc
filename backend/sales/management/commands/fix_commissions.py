...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        written, skipped, unknown_currencies = 0, Counter(), Counter()
        ...
        for txn in transactions:
            ...
            amount_usd = self.invoice_usd(txn)
        ...
        self.report(written, skipped, unknown_currencies, dry_run)

    @staticmethod
    def invoice_usd(txn: Transaction) -> Decimal | None:
        """
        What the invoice was worth in fiat.

        The invoice's own `source_price` first - it is the number Plisio billed - and the order's
        total as the fallback, which is what we asked Plisio to bill in the first place.
        """

        if txn.source_price is not None and str(txn.source_price.currency) == "USD":
            return txn.source_price.amount

        if txn.order_id and txn.order.total_price is not None:
            return txn.order.total_price.amount

        return None

    def report(self, written: int, skipped: Counter, unknown_currencies: Counter, dry_run: bool):
