...
Записаны по состоянию на день инцидента. Что с ними стало:

- ~~**Согласовать cron и порог истечения**~~ — сделано: `backend/cronjob` гоняет
  `expire_transactions` каждые 10 минут, а не раз в сутки.
- ~~**Долгосрочно — Вариант 1 (Atomic Link)**~~ — сделано, но иначе: связь живёт не полем на файле,
  а отдельной моделью `Allocation` с частичным уникальным индексом
  ([ADR-0001](../adr/0001-allocation-as-single-source-of-truth.md), релиз
  [R1](../db-refactoring/r1.md)). Заметки с вариантами схемы удалены, решения — в `docs/adr/`.
  Гонка, из которой вырос инцидент, стала невозможной на уровне схемы.
- ~~**Мониторинг «PAID без ссылок»**~~ — состояние перестало существовать: `Order.deliver()`
  идемпотентен и до-аллоцирует недостающие единицы, а если стока нет — callback откатывается
  целиком и отвечает 409, вместо того чтобы оставить заказ оплаченным и невыданным.
- **Открыто: `expire_min`.** Инвойс Plisio по-прежнему живёт 60 минут, а подтверждение
  крипто-транзакции может занять часы. Поздний платёж теперь самовосстанавливается (единицы
  перевыдаются из текущего стока), так что это больше не потеря заказа — но резерв всё равно
  отпускается раньше, чем приходят деньги, и при пустом стоке покупатель упирается в 409.
